...
# Car class
class Car:
    def __init__(self, x, y, image, speed=car_initial_speed):
        self.x = x
        self.y = y
        self.speed = speed
        self.image = image
...
def generate_opponent_cars(num_cars, speed):
    cars = []
    while len(cars) < num_cars:
        ...
        if not overlap:
            new_car = Car(new_x, new_y, random.choice([blue_car_img, yellow_car_img, green_car_img]), speed)
            cars.append(new_car)
    return cars

# Function to respawn opponent car without overlap
def respawn_opponent_car(car, cars, speed):
    while True:
        new_x = random.randint(int(TRACK_MARGIN), int(WIDTH - TRACK_MARGIN - CAR_WIDTH))
        new_y = random.randint(-600, -100)
        overlap = False
...
score_bg_img = pygame.transform.scale(score_bg_img, (220, 230))  # Keep the size as 150x230

# Function to display real-time score with a background
def show_score(screen, score, font, speed):
    score_bg_x, score_bg_y = 30, -80  # Adjusted closer to the top
    shown_speed = int(speed * 10)
    score_text = font.render(f"{int(score)} m {shown_speed} m/h", True, (255, 255, 255))
...
def run_game():
    ...
    # Initialize speed to the starting value
    game_speed = car_initial_speed

    # Reset the game state
    player_car = Car(WIDTH // 2, HEIGHT - 120, player_car_img, game_speed)
    opponent_cars = generate_opponent_cars(6, game_speed)  # Pass initial speed
    ...
    while True:
        ...
        if running:
            ...
            if round(round(float(distance), 5)%100, 5) == 0.1 and distance != 0:
                game_speed += 1  # Increase opponent car speed

            # Update opponent car speeds
            for opponent in opponent_cars:
                opponent.speed = game_speed
            ...
            for opponent in opponent_cars:
                ...
                if opponent.y > HEIGHT:
                    respawn_opponent_car(opponent, opponent_cars, game_speed)  # Respawn car without overlap
            ...
            show_score(screen, distance, font, game_speed)
